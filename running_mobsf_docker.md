...
You must run any of the **[supported](dynamic_analyzer_docker.md)** Android VM/emulator and obtain the `MOBSF_ANALYZER_IDENTIFIER` before running MobSF. For iOS, use Corellium or a jailbroken iOS device.
...
Connect MobSF and the jailbroken iOS device to same WiFi/LAN. Use the device’s Wi‑Fi IP and SSH port.
...
Details and proxy setup: [Dynamic Analyzer — Jailbroken iOS device](dynamic_analyzer_docker.md#jailbroken-ios-device-ssh).
