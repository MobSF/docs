...
3. `Viewer`: This role is read-only and can only view scan results without performing a new scan.
...
You would need the following things from MobSF to configure it in Okta.
...
You would need the following things from Okta to Configure in MobSF.
...
6. Under the `Attribute Statements (optional)` section, create a new attribute statement to the user email to MobSF.
    * **Name**: `email`
...
9. You must create at least two Okta groups for the MobSF roles `Maintainer` and `Viewer`. The group name should contain the string `maintainer` in it to be associated with the `Maintainer` role, and any other group name will be assigned to the Read-Only `Viewer` role.
...
11. You can add corresponding users to each group.

12. Now Go to the **Sign On** tab of the created app and copy the Metadata URL.
