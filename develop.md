...
You need one of the following Android/iOS targets for Dynamic Analysis.
...
!> Dynamic analysis using a **non-jailbroken** real phone is not supported. For Android, dynamic analysis on a rooted physical device is possible but **not supported by us.**
...
!> Non jailbroken devices cannot be used with MobSF. For physical iPhones, use a jailbroken device.
...
Use a physical jailbroken device with SSH and a compatible **Frida server** (e.g. **>17.0.0** from MobSF v4.5.1 onwards).
